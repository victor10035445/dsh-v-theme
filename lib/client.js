 ...
 *
 * 主题（v0.5.0）：
 *  - 合成波 '84（Synthwave）：霓虹粉 × 电光蓝 × 紫罗兰夜空 + 地平线辉光
 *  - 融合战术（Fusion）：SOC 战术面板基底（AAA 对比度）+ FUI 霓虹强调
 *    （青 #56d4e0 主操作 / 绿 #00FF88 健康 / 黄 #f0c239 警示 / 赤红 #FF3366），
 *    参照 demo5-fusion.html 的融合设计，顶部青色径向辉光 + HUD 扫描线
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    /**
     * 合成波 '84 特效层（仅主题开启时挂载；body.v-cyber 作用域）。
     * 霓虹粉 / 电光蓝 / 紫罗兰；底部地平线辉光是本主题的签名。
     */
    const FX_SYNTHWAVE = `/* dsh-v-theme: 合成波特效层 */
body.${BODY_CLASS}{
...
@media (prefers-reduced-motion:reduce){
  ...
  body.${BODY_CLASS} [data-streaming]::before{animation:none}
    ...
    /**
     * 融合战术特效层：SOC 基底 + FUI 强调（参照 demo5-fusion.html）。
     * 顶部青色径向辉光（对应 demo5 Hero），青色系扫描线与辉光。
     */
    const FX_FUSION = `/* dsh-v-theme: 融合战术特效层 */
body.${BODY_CLASS}{
...
.v-hud .v-hudDim{opacity:.55}
/* CRT 滚动亮带已移除（清晰度优先） */
/* ═══ Fusion 深度造型：SOC 克制 + FUI 点睛（切角 / HUD 角标 / DEFCON 状态） ═══ */
...
/* 渐变顶边：border-top 无法用渐变，用内嵌伪元素画（demo5 badge-fusion 配方） */
...
/* P0 · 工具卡：demo5 .panel 配方（半透明 + blur）+ 渐变 DEFCON 状态顶边 + neon-pulse
   伪元素分工：卡片 ::before = 渐变顶边；外层 flowItem ::after = HUD 角标（::before 留给流式扫描线）
   运行中 = 青→青蓝 / 结果 = 绿→青 / 出错 = 赤红→品红（demo5 多色顶边编码完整版） */
body.${BODY_CLASS} [data-chat-flow-kind="tool-call"] [class*="callRow"],
body.${BODY_CLASS} [data-chat-flow-kind="tool-call"] > div > div,
body.${BODY_CLASS} [data-chat-flow-kind="tool-result"] [class*="callRow"],
body.${BODY_CLASS} [data-chat-flow-kind="tool-result"] > div > div{
...
/* HUD L 型角标：挂在外层 flowItem 的 ::after（左上/右下角线，demo5 .hud-corner 缩版） */
...
@media (prefers-reduced-motion:reduce){
  ...
  body.${BODY_CLASS} [data-streaming]::before{animation:none}
    ...
    /** 合成波 '84 调色板：粉 / 粉蓝强调层（背景族全部走 DSH 原生配色）。 */
    const PALETTE_SYNTHWAVE = {
      /* 边框：粉调 alpha 线 */
    ...
    /** 融合战术调色板：SOC 青 / FUI 强调层（背景族全部走 DSH 原生配色）。 */
    const PALETTE_FUSION = {
    ...
    const THEMES = {
      synthwave: {
        id: "synthwave",
        labelKey: "v.theme.synthwave",
        hudBrand: "█ V://SYNTHWAVE █",
        swatch: ["#FF2E97", "#FF5CB0", "#4DA8FF"],
        palette: PALETTE_SYNTHWAVE,
        fx: FX_SYNTHWAVE,
        tokens: buildTokens(PALETTE_SYNTHWAVE)
      },
      fusion: {
        id: "fusion",
        labelKey: "v.theme.fusion",
        hudBrand: "█ V://FUSION █",
        swatch: ["#56D4E0", "#00FF88", "#F0C239"],
        palette: PALETTE_FUSION,
        fx: FX_FUSION,
        tokens: buildTokens(PALETTE_FUSION)
      }
    };
    const THEME_LIST = [THEMES.synthwave, THEMES.fusion];
...
@media (prefers-reduced-motion:reduce){.vOn .vState::after{animation:none}}`;
    ...
    const zh = {
      ...
      "v.theme.synthwave": "合成波 '84",
      "v.theme.fusion": "融合战术",
      "v.glowLabel": "氛围 AMBIENCE",
      "v.glow": "边缘辉光",
      "v.desc": "V 赛博主题：两套风格可切换——合成波 '84（霓虹粉 × 电光蓝 × 地平线辉光）与融合战术（SOC 战术面板基底 + FUI 青色霓虹）。侧栏会话行随官方状态点亮框体：进行中=青/粉蓝、待交互=黄/金、完成=亮绿、空闲静默退场。开启期间固定深色基底；在「外观」里切走深色会自动断开。"
    };
    const en = {
      "v.nav": "Skin",
      "v.title": "V CYBER",
